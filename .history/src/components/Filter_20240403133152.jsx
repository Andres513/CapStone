    ...
    return (
        <div>    
            ...
            {categories.map((category, index) => (
                <button key={index} onClick={() => handleCategoryFilter('category', category)} className="dot">{category}</button>
            ))}
            </div>
                <div>
                    {priceRange.map((price, index) => (
                        <button key={index} onClick={() => handlePriceFilter('price', price)} className="dot">{price}</button>
                    ))}
                </div>
        </div>
    )
